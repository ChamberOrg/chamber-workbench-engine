...
**SDK-named token (`deepseek-harness-sdk`).** Considered for the `User-Agent` token because the supported runtime client stack uses the SDK name. `deepseek-harness` won because it names the Chamber product, matches the org/repo identity and package scope, and keeps wire attribution stable without calling the complete product an SDK.
