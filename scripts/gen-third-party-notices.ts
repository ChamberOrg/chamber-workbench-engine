...
function renderNonPermissiveNote(deps: ExternalDep[]): string {
  ...
  return `\n${subject} ${named.length === 1 ? 'runs' : 'run'} only as development tooling; their code is not linked into or distributed with any Chamber artifact.\n`
}
...
Chamber is licensed under [MIT](LICENSE). It depends on the third-party software listed below. Each project remains under its own license; nothing in this file changes those terms.
